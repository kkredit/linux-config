...
function vo {
	nvim -c "Telescope find_files find_command=echo,$(lastfiles) layout_strategy=horizontal layout_config={height=50}"
}
...
function serial() {
	...
	while getopts "hb:n:" opt; do
		case $opt in
		h)
			echo "Usage: serial (-b BAUD_RATE) (-n DEVICE_NUM)"
			return
			;;
		b) BAUD=$OPTARG ;;
		n)
			DEV_NUM=$OPTARG
			echo "Waiting for device $OPTARG"
			;;
		*)
			echo "Invalid option"
			return 2

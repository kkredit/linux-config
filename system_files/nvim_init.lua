...
local on_attach = function(_, bufnr) -- (client, bufnr)
  ...
  buf_set_keymap('n', '<leader>le',
    '<cmd>lua vim.diagnostic.jump({severity={min=vim.diagnostic.severity.ERROR},count=1,float=true})<CR>', opts)
  buf_set_keymap('n', '<leader>lN', '<cmd>lua vim.diagnostic.jump({count=-1,float=true})<CR>', opts)
  buf_set_keymap('n', '<leader>ln', '<cmd>lua vim.diagnostic.jump({count=1,float=true})<CR>', opts)
